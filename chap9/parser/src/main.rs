...
impl fmt::Display for ParseError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        use self::ParseError::*;
        match self {
            UnexpectedToken(tok) => write!(f, "{}: {} is no expected", tok.loc, tok.value),
            NotExpression(tok) => write!(f,
                                         "{}: '{}' is not a start of expression",
                                         tok.loc, tok.value,
            ),
            NotOperator(tok) => write!(f,
                                       "{}: '{}' is not an operator",
                                       tok.loc, tok.value,
            ),
            UnclosedOpenParen(tok) => write!(f,
                                             "{}: '{}' is not closed",
                                             tok.loc, tok.value,
            ),
            RedundantExpression(tok) => write!(f,
                                               "{}: expression after '{}' is redunndant",
                                               tok.loc, tok.value,
            ),
            Eof => write!(f, "End of file"),
        }
...
impl Interpreter {
    ...
    pub fn eval(&mut self, expr: &Ast) -> Result<i64, InterpreterError> {
        ...
        match expr.value {
            ...
            AstKind::BinOp { ref op, ref l, ref r } => {
                let l = self.eval(l)?;
                let r = self.eval(r)?;
...
impl StdError for InterpreterError {
    fn description(&self) -> &str {
        use self::InterpreterErrorKind::*;
        match self.value {
            InterpreterErrorKind::DivisionByZero => "the right hand expression of the division evaluates to zero",
        }
    }
}
...
impl RpnCompiler {
    ...
    pub fn compile_inner(&mut self, expr: &Ast, buf: &mut String) {
        use self::AstKind::*;
        match expr.value {
            AstKind::Num(n) => buf.push_str(&n.to_string()),
            AstKind::UniOp { ref op, ref e } => {
    ...
    fn compile_uniop(&mut self, op: &UniOp, buf: &mut String) {
        use self::UniOpKind::*;
        match op.value {
            UniOpKind::Plus => buf.push_str("+"),
            UniOpKind::Minus => buf.push_str("-"),
        }
    }
    fn compile_binop(&mut self, op: &BinOp, buf: &mut String) {
        use self::BinOpKind::*;
        match op.value {
            BinOpKind::Add => buf.push_str("+"),
            BinOpKind::Sub => buf.push_str("-"),
            BinOpKind::Mult => buf.push_str("*"),
            BinOpKind::Div => buf.push_str("/"),
        }
    }
}
